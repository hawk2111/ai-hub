...
budget. Reads, tests and `agentguard reset` stay available, so there is no deadlock.

```bash
pip install -e .
agentguard install            # wires hooks into all three CLIs, scaffolds agentguard.toml
agentguard status             # breaker + budget, as JSON
agentguard trip "investigating"
agentguard reset              # closing the breaker is always a human decision
```

Install once; run `agentguard install` in each repository. It generates the hook
...
including your own, dropped into `.agentguard/conditions/`.

## Configuration

`.agentguard/agentguard.toml`, overridden by `AGENTGUARD_*` environment variables so
...
Env: `AGENTGUARD_MAX_STEPS`, `AGENTGUARD_MAX_TOKENS`, `AGENTGUARD_BREAKER_THRESHOLD`,
`AGENTGUARD_MAX_RATE_PERCENT`, `AGENTGUARD_SKIP_BUDGET`, `AGENTGUARD_SKIP_GATE`,
`AGENTGUARD_HOME`, `AGENTGUARD_PROJECT_DIR`, `AGENTGUARD_GC_DAYS`.

## Design decisions worth knowing

**An internal exception exits 0.** Copilot treats any non-zero exit other than 2 as a
*deny*, while Claude treats it as a harmless error. A crash in agentguard would not
...
`agentguard reset`. The budget ledger, which only ever *informs* the breaker, starts
fresh instead.
...
finishing. Otherwise a typo in `agentguard.toml` would open the breaker after three
turns.
...
|---|---|---|---|---|
| config | `.claude/settings.json` | `.codex/hooks.json` | `.github/hooks/agentguard.json` | reuses the other two |
...
- **Codex** only fires `PreToolUse` for `apply_patch` since
  ...
  older builds only `Bash` is gated and file edits slip past. `agentguard install`
...
Everything agentguard runs is Python. It ships no `.sh`, no `.ps1`, and no shell
...
| host | how agentguard is invoked | shell involved |
|---|---|---|
...
Elsewhere the host insists on a command string; what agentguard puts in it is a bare
...
**The agent's own shell tool is not blocked.** It cannot be — `agentguard reset` has to
