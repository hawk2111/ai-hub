"""Configuration: TOML on disk, `AGENTGUARD_*` on top.
...
def project_dir() -> Path:
    for var in ("AGENTGUARD_PROJECT_DIR", "CLAUDE_PROJECT_DIR"):
...
def home_dir(project: Path | None = None) -> Path:
    value = os.environ.get("AGENTGUARD_HOME")
    if value:
        return Path(value)
    return (project or project_dir()) / ".agentguard"
...
def _pick_int(env_name: str, raw: dict[str, Any], key: str, default: int) -> int:
    ...
    `_env_int(...) or file_value` would quietly ignore `AGENTGUARD_GC_DAYS=0`, which
...
def _apply_env(conditions: list[dict[str, Any]]) -> None:
    overrides = (
        ("AGENTGUARD_MAX_STEPS", "step_budget", "max_steps", _env_int),
        ("AGENTGUARD_MAX_TOKENS", "token_budget", "max_tokens", _env_int),
        ("AGENTGUARD_BREAKER_THRESHOLD", "gate_failures", "threshold", _env_int),
        ("AGENTGUARD_MAX_RATE_PERCENT", "rate_limit_pressure", "max_percent", _env_float),
    )
...
def load() -> Config:
    ...
    data: dict[str, Any] = {}
    data = _merge(data, _read_toml(Path.home() / ".agentguard" / CONFIG_NAME))
    ...
    budget = BudgetConfig(
        token_source=os.environ.get("AGENTGUARD_TOKEN_SOURCE")
        or str(budget_raw.get("token_source", "auto")),
        recompute_every=_pick_int("AGENTGUARD_RECOMPUTE_EVERY", budget_raw, "recompute_every", 5),
    )
    ...
    return Config(
        ...
        skip_budget=os.environ.get("AGENTGUARD_SKIP_BUDGET") == "1",
        skip_gate=os.environ.get("AGENTGUARD_SKIP_GATE") == "1",
        gc_days=_pick_int("AGENTGUARD_GC_DAYS", data, "gc_days", DEFAULT_GC_DAYS),
        max_sessions=_pick_int(
            "AGENTGUARD_MAX_SESSIONS", data, "max_sessions", DEFAULT_MAX_SESSIONS
        ),
    )
