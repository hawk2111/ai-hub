...
from agentguard import audit, gate
from agentguard.breaker import Breaker
from agentguard.budget import Budget, SessionBudget
from agentguard.conditions import Condition, EvalContext, Trip, build_all, evaluate, load_custom
from agentguard.config import Config
from agentguard.events import ALLOW, NOOP, Decision, EventType, HookEvent, block_stop, deny
from agentguard.state import Store
from agentguard.tokens import get_source
...
def pre_tool_use(event: HookEvent, rt: Runtime) -> Decision:
    ...
    trip = evaluate(
        ...
        EvalContext(
            ...
            consecutive_fails=status.fails,
        ),
    )
    if trip is None:
        return ALLOW

    _open(event, rt, trip, detail={"steps": snapshot.steps, "tokens": snapshot.tokens})
...
def _handle_red_gate(event: HookEvent, rt: Runtime, failures: list[gate.Failure]) -> Decision:
    ...
    if trip is not None:
        ...
        _open(event, rt, trip, detail={"fails": fails, "gate": message})
...
def _refresh_budget(event: HookEvent, rt: Runtime) -> SessionBudget:
    ...
    source = get_source(rt.config.budget.token_source, event.provider)
    return rt.budget.refresh(event, source)
...
def _deny_write(event: HookEvent, rt: Runtime, reason: str) -> Decision:
    """Deny only mutations. Reads, tests and `agentguard reset` stay reachable."""
    ...
    return deny(
        f"[agentguard] BLOCKED: circuit breaker is OPEN ({reason}). "
        "The run is read-only until a human investigates. "
        "Reset with: agentguard reset"
    )
