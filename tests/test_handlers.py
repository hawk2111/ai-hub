...
def test_step_budget_trips_the_breaker_and_denies_the_offending_write(
    ...
    monkeypatch.setenv("AGENTGUARD_MAX_STEPS", "2")
...
def test_a_tripped_breaker_still_lets_reads_through(project, state_dir, monkeypatch, capsys):
    monkeypatch.setenv("AGENTGUARD_MAX_STEPS", "1")
...
def test_a_trip_is_written_to_the_audit_log(project, monkeypatch, capsys):
    ...
    lines = (project / ".agentguard" / "state" / "audit.jsonl").read_text().strip().splitlines()
...
def test_an_unlaunchable_check_never_marches_the_breaker_toward_its_threshold(
    ...
    """A typo in agentguard.toml (or `npm` on Windows) is a config error, not red code."""
    write_config(
        project,
        """
        [[conditions]]
    ...
    entries = (project / ".agentguard" / "state" / "audit.jsonl").read_text()
