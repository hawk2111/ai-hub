...
def test_open_breaker_allows_reads(provider, project, state_dir, monkeypatch, capsys):
    ...
    assert code == 0, "reads and `runbreaker reset` must stay reachable"
...
def test_internal_exception_never_fails_closed(provider, project, monkeypatch, capsys):
    def boom(*_args: object, **_kwargs: object) -> None:
        raise RuntimeError("bug in runbreaker")

    monkeypatch.setattr(hook, "route", boom)
    code, _, _ = run_pre(provider, monkeypatch, capsys)
    assert code == 0, "a bug in runbreaker must never lock a Copilot user out"
...
def test_copilot_block_stop_uses_json_body(project, monkeypatch, capsys):
    ...
    from runbreaker.events import block_stop
    from runbreaker.providers import CopilotAdapter
...
def test_shim_path_runs_as_a_real_process(project: Path) -> None:
    """The generated shim must import runbreaker without a venv on sys.path."""
    import json
    import subprocess
    import sys
    ...
    proc = subprocess.run(
        ...
        env={
            "PATH": "/usr/bin:/bin",
            "RUNBREAKER_HOME": str(cfg.home),
            "RUNBREAKER_PROJECT_DIR": str(project),
        },
        check=False,
    )
