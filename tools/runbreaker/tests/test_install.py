...
from runbreaker import config, install
...
def test_claude_exec_form_keeps_the_shim_path_relocatable(project):
    ...
    assert entry["args"][1].endswith(".runbreaker/shims/claude_stop.py")


def test_claude_exec_form_falls_back_to_an_absolute_path_outside_the_project(
    tmp_path, monkeypatch
):
    monkeypatch.setenv("RUNBREAKER_PROJECT_DIR", str(tmp_path / "proj"))
    monkeypatch.setenv("RUNBREAKER_HOME", str(tmp_path / "elsewhere"))
    entry = install._exec_entry(config.load(), "claude", "stop")
...
def test_no_config_ever_names_a_shell(project):
    ...
    for rel in (".claude/settings.json", ".codex/hooks.json", ".github/hooks/runbreaker.json"):
...
def test_copilot_uses_the_shell_neutral_command_field(project):
    ...
    data = json.loads((project / ".github" / "hooks" / "runbreaker.json").read_text())
...
def test_paths_containing_spaces_are_quoted(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("RUNBREAKER_HOME", str(home))
...
def test_a_spaced_path_forces_a_powershell_override_with_the_call_operator(
    ...
    monkeypatch.setenv("RUNBREAKER_HOME", str(tmp_path / "Program Files" / "guard"))
    monkeypatch.setenv("RUNBREAKER_PROJECT_DIR", str(tmp_path))
    install.install(config.load(), ("copilot",))
    entry = json.loads((tmp_path / ".github" / "hooks" / "runbreaker.json").read_text())[
        "hooks"
    ]["PreToolUse"][0]
...
def test_a_space_free_path_needs_no_powershell_override(project):
    install.install(config.load(), ("copilot",))
    entry = json.loads((project / ".github" / "hooks" / "runbreaker.json").read_text())["hooks"][
        "PreToolUse"
    ][0]
...
def test_gitignore_gets_the_generated_paths(project):
    ...
    assert ".runbreaker/state/" in ignored
    assert ".runbreaker/shims/" in ignored
