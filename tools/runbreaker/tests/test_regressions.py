...
from runbreaker.tokens import base
from runbreaker.tokens.claude import ClaudeTokenSource, usage_tokens
...
def test_an_unlaunchable_check_does_not_block_the_stop(project, monkeypatch, capsys):
    ...
    assert code == 0, "a typo in runbreaker.toml is not broken code to fix"
...
def test_copilot_stop_timeout_covers_the_gate(project):
    ...
    hooks = json.loads((project / ".github" / "hooks" / "runbreaker.json").read_text())["hooks"]
...
def test_the_gate_output_survives_into_the_audit_when_the_breaker_trips(
    ...
    trips = [
        json.loads(line)
        for line in (project / ".runbreaker" / "state" / "audit.jsonl").read_text().splitlines()
...
def test_install_scaffolds_a_starter_config(project):
    ...
    body = (project / ".runbreaker" / "runbreaker.toml").read_text()
...
def test_install_never_overwrites_a_tuned_config(project):
    ...
    install.install(config.load(), ("claude",))
    assert (project / ".runbreaker" / "runbreaker.toml").read_text() == "# mine\n"


def test_install_leaves_a_foreign_hook_that_merely_mentions_runbreaker(project):
    settings = project / ".claude" / "settings.json"
    settings.parent.mkdir(parents=True)
    settings.write_text(
        json.dumps({"hooks": {"PreToolUse": [{"hooks": [{"command": "echo runbreaker is nice"}]}]}})
    )
    install.install(config.load(), ("claude",))
...
def test_a_token_budget_can_trip_at_stop_even_with_no_gate(project, state_dir, monkeypatch, capsys):
    ...
    monkeypatch.setenv("RUNBREAKER_MAX_TOKENS", "1000")
